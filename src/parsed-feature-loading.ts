...
const parseScenarioOutlineExampleSet = (astExampleSet: any, outlineScenario: ParsedScenario) => {
  ...
  return exampleTable.map(tableRow => parseScenarioOutlineExample(tableRow, outlineScenario, parseTags(astExampleSet)));
};
...
const collapseRulesAndBackgrounds = (astFeature: any) => {
  ...
  const children = collapseBackgrounds(astFeature.children, featureBackgrounds).reduce(
    (newChildren: [], nextChild: any) => {
      if (nextChild.rule) {
        const { rule } = nextChild;
        const ruleBackgrounds = parseBackgrounds(rule);

        return [...newChildren, ...collapseBackgrounds(rule.children, [...featureBackgrounds, ...ruleBackgrounds])];
      }
