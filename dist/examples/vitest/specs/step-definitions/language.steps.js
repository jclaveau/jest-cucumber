...
var feature = (0, src_1.loadFeature)('./examples/typescript/specs/features/language.feature');
(0, src_1.defineFeature)(feature, function (test) {
    ...
    test('Invullen van een correct wachtwoord', function (_a) {
        ...
        then('krijg ik toegang', function () {
            (0, vitest_1.expect)(accessGranted).toBeTruthy();
        });
    });
    ...
    test('een artikel toevoegen aan mijn takenlijst', function (_a) {
        ...
        then('zou ik de volgende takenlijst zien:', function (table) {
            ...
            table.forEach(function (_, index) {
                (0, vitest_1.expect)(todoList.items[index].name).toBe(table[index].TaakNaam);
                (0, vitest_1.expect)(todoList.items[index].priority).toBe(table[index].Prioriteit);
            });
        });
    });
});
