...
var feature = (0, src_1.loadFeature)('./examples/typescript/specs/features/using-latest-gherkin-keywords.feature');
(0, src_1.defineFeature)(feature, function (test) {
    ...
    var thenTheOutputOfXShouldBeDisplayed = function (then) {
        then(/^the output of "(\w+)" should be displayed$/, function (expectedOutput) {
            if (!expectedOutput) {
                (0, vitest_1.expect)(output).toBeFalsy();
            }
            else {
                (0, vitest_1.expect)(output).toBe(parseFloat(expectedOutput));
            }
        });
    };
